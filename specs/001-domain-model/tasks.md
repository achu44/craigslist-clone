...
- [x] **T001** Create `apps/__init__.py` and the four app packages (`apps/geo`,
...
- [x] **T002** Register all four apps in `config/settings.py` `INSTALLED_APPS`.
...
- [x] **T003** `apps/geo/models.py`: `Site` and `Area` models exactly as specified in
...
- [x] **T004** `apps/taxonomy/models.py`: `Channel` and `Category` models, including the
...
- [x] **T005** `apps/postings/models.py`: `Posting` model, including `submitted_at`
...
- [x] **T009** `admin.py` in each of the four apps: register every model with
...
- [x] **T010** Add `FLAG_HIDE_THRESHOLD` to `config/settings.py` (env-backed, default
...
- [x] **T011** `apps/postings/exceptions.py`: `InvalidTransition`, `RenewalTooSoon`,
...
- [x] **T012** `apps/postings/services.py`: manage-token helpers —
...
- [x] **T013** `apps/postings/models.py`: `PostingQuerySet.visible()` and wire it as
...
- [x] **T014** `create_posting(...)` → `DRAFT`. Validates: category is a leaf
...
- [x] **T015** `submit_posting(posting)`: `DRAFT` → `PENDING`, sets `submitted_at`.
  Rejects from any other status (`InvalidTransition`).

- [x] **T016** `confirm_posting(posting)`: `PENDING` → `ACTIVE` within
...
- [x] **T017** `renew_posting(posting)`: `ACTIVE`/`EXPIRED` → `ACTIVE` (confirmed
...
- [x] **T018** `delete_posting(posting)`: `DRAFT`/`PENDING`/`ACTIVE`/`EXPIRED` →
  `DELETED`, sets `removed_at`. Rejects from `BLOCKED` and `HIDDEN`
  (`InvalidTransition`).

- [x] **T019** `expire_active_postings()`: bulk `ACTIVE` → `EXPIRED` where `expires_at`
  has passed. Bulk `update()`, no per-row exceptions.

- [x] **T020** `hide_posting(posting)`: `ACTIVE` → `HIDDEN`, sets `removed_at`. Rejects
  from any other status (`InvalidTransition`).

- [x] **T021** `block_posting(posting)`: any status except `DELETED` → `BLOCKED`, sets
  `removed_at`. Rejects from `DELETED` (`InvalidTransition`). No caller wired yet
  (confirmed — plan.md §5).

- [x] **T022** `repost_posting(source, token)`: validates `token` against `source`
...
- [x] **T023** `add_posting_image(posting, image)`: rejects the 25th image
...
- [x] **T024** `apps/moderation/services.py`: `flag_posting(posting, reason,
...
- [x] **T025** `apps/postings/management/commands/expire_postings.py`: thin wrapper
...
- [x] **T026** `tests/factories.py`: `make_site`, `make_area`, `make_channel`,
...
- [x] **T027** `tests/postings/test_models.py`: negative price is rejected at the DB
...
- [x] **T028** `tests/taxonomy/test_models.py`: assigning a posting to a non-leaf
...
- [x] **T029** Run the full suite and report status:
  - `uv run pytest`
