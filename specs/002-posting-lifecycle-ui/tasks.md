...
- [x] **T001** Add `django_htmx` to `INSTALLED_APPS` and `django_htmx.middleware.HtmxMiddleware`
  to `MIDDLEWARE` in `config/settings.py`.
  **Verify:** `uv run python manage.py check` passes with no missing-app/middleware errors.

- [x] **T002** Set `TEMPLATES[0]["DIRS"] = [BASE_DIR / "templates"]` and add
...
- [x] **T003** Create `apps/postings/urls.py` (empty `urlpatterns = []`, `app_name = "postings"`
...
- [x] **T004** Add a minimal `static/css/base.css`. No htmx file is vendored into this
...
- [x] **T005** `templates/base.html`: shared layout loading `static/css/base.css` and
...
- [x] **T006** `apps/postings/forms.py`: `PostingCreateForm` exactly as specified in
...
- [x] **T007** `apps/postings/views.py`: `browse(request, site_slug)` — 404 for an
...
- [x] **T008** `detail(request, site_slug, public_id)` — 404 unless the posting is in
...
- [x] **T009** `templates/postings/browse.html` and `templates/postings/detail.html`,
  extending `base.html`.
...
- [x] **T010** `create(request, site_slug)` — GET renders `PostingCreateForm`; POST calls
...
- [x] **T011** `price_field_partial(request, site_slug)` — HTMX endpoint rendering
...
- [x] **T012** `templates/postings/create.html` — the form, with the category `<select>`
...
- [x] **T013** `submitted(request, site_slug, public_id, token)` — 404 via
...
- [x] **T014** `confirm(request, site_slug, public_id, token)` — calls `confirm_posting()`;
...
- [x] **T015** `templates/postings/submitted.html` and `templates/postings/confirm_error.html`.
...
- [x] **T016** `manage(request, site_slug, public_id, token)` — 404 via
...
- [x] **T017** `manage_delete(request, site_slug, public_id, token)` (`@require_POST`) —
...
- [x] **T018** `manage_renew(request, site_slug, public_id, token)` (`@require_POST`) —
...
- [x] **T019** `templates/postings/manage.html` — status, expiry, renew/delete forms with
...
- [x] **T020** `apps/geo/management/commands/seed_dev_data.py` exactly as specified in
...
- [x] **T021** Run the full suite and report status:
